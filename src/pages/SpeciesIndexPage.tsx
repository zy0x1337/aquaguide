...
import {
  ...
  ArrowDownAZ, ArrowDownZA, ArrowDown01, ArrowDown10, Sprout, Flame, Minimize2, Maximize2, Ruler
} from 'lucide-react';
...
const SortIcon = ({ value, className = 'w-3.5 h-3.5' }: { value: SortOption; className?: string }) => {
  switch (value) {
    case 'default':         return <Sparkles className={className} />;
    case 'name-asc':        return <ArrowDownAZ className={className} />;
    case 'name-desc':       return <ArrowDownZA className={className} />;
    case 'size-asc':        return <ArrowDown01 className={className} />;
    case 'size-desc':       return <ArrowDown10 className={className} />;
    case 'difficulty-asc':  return <Sprout className={className} />;
    case 'difficulty-desc': return <Flame className={className} />;
    case 'tank-asc':        return <Minimize2 className={className} />;
    case 'tank-desc':       return <Maximize2 className={className} />;
    default:                return <ArrowUpDown className={className} />;
  }
};
...
const SpeciesIndexPage = () => {
  ...
  const applyFilters = (species: Species): boolean => {
    ...
    if (filters.biotope) {
      ...
      if (!bio.includes(filters.biotope.toLowerCase())) return false;
    }
    if (filters.type) {
    ...
    if (filters.behaviorTags.length > 0) {
      if (!filters.behaviorTags.every(tag => species.behavior.tags.includes(tag))) return false;
    }
    return true;
  };
  ...
  const applySorting = (species: Species[]): Species[] => {
    ...
    return [...species].sort((a, b) => {
      switch (sortBy) {
        case 'name-asc':        return a.taxonomy.commonName.localeCompare(b.taxonomy.commonName);
        case 'name-desc':       return b.taxonomy.commonName.localeCompare(a.taxonomy.commonName);
        case 'size-asc':        return a.visuals.adultSizeCM - b.visuals.adultSizeCM;
        case 'size-desc':       return b.visuals.adultSizeCM - a.visuals.adultSizeCM;
        case 'difficulty-asc':  return (DIFFICULTY_ORDER[a.care.difficulty] ?? 1) - (DIFFICULTY_ORDER[b.care.difficulty] ?? 1);
        case 'difficulty-desc': return (DIFFICULTY_ORDER[b.care.difficulty] ?? 1) - (DIFFICULTY_ORDER[a.care.difficulty] ?? 1);
        case 'tank-asc':        return a.environment.minTankSizeLiters - b.environment.minTankSizeLiters;
        case 'tank-desc':       return b.environment.minTankSizeLiters - a.environment.minTankSizeLiters;
        default:                return 0;
      }
    });
  };
  ...
  const filteredSpecies = useMemo(() => {
    ...
    if (searchTerm.trim()) results = fuse.search(searchTerm).map(r => r.item);
    const filtered = results.filter(applyFilters);
    const unique = Array.from(new Map(filtered.map(s => [s.id, s])).values());
    return applySorting(unique);
  }, [searchTerm, filters, fuse, sortBy]);

  useEffect(() => { setDisplayCount(INITIAL_LOAD); }, [filteredSpecies]);

  const displayedSpecies = useMemo(() => filteredSpecies.slice(0, displayCount), [filteredSpecies, displayCount]);
  ...
  const resetFilters = () => {
    setFilters({ level: null, region: null, tankSize: null, biotope: '', type: null, tempMin: 15, tempMax: 30, phMin: 5.0, phMax: 9.0, maxBodySize: 100, diet: 'all', temperament: 'all', behaviorTags: [] });
    setSearchTerm('');
    setSortBy('default');
    ...
    <div className="space-y-6">
      <div className="space-y-5">
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <FilterChip label="Beginner" icon={<Sparkles className="w-3 h-3 md:w-3.5 md:h-3.5" />} isActive={filters.level === 'beginner'} onClick={() => setFilters({ ...filters, level: filters.level === 'beginner' ? null : 'beginner' })} colorClass="text-emerald-700 bg-emerald-50 border-emerald-200 hover:bg-emerald-100 dark:text-emerald-300 dark:bg-emerald-950/30 dark:border-emerald-800" activeClass="bg-emerald-600 text-white border-emerald-700 shadow-lg" />
            <FilterChip label="Medium"   icon={<TrendingUp className="w-3 h-3 md:w-3.5 md:h-3.5" />} isActive={filters.level === 'medium'}   onClick={() => setFilters({ ...filters, level: filters.level === 'medium' ? null : 'medium' })}   colorClass="text-amber-700 bg-amber-50 border-amber-200 hover:bg-amber-100 dark:text-amber-300 dark:bg-amber-950/30 dark:border-amber-800"   activeClass="bg-amber-600 text-white border-amber-700 shadow-lg" />
            <FilterChip label="Expert"   icon={<Flame className="w-3 h-3 md:w-3.5 md:h-3.5" />}     isActive={filters.level === 'expert'}   onClick={() => setFilters({ ...filters, level: filters.level === 'expert' ? null : 'expert' })}     colorClass="text-rose-700 bg-rose-50 border-rose-200 hover:bg-rose-100 dark:text-rose-300 dark:bg-rose-950/30 dark:border-rose-800"       activeClass="bg-rose-600 text-white border-rose-700 shadow-lg" />
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {regions.map(region => (
              <button key={region} onClick={() => setFilters({ ...filters, region: filters.region === region ? null : region })} className={cn("px-3 py-2 rounded-lg md:rounded-xl text-[10px] font-bold uppercase tracking-tight border-2 transition-all duration-200 text-center leading-[1.3] min-w-[70px] flex-1", filters.region === region ? "bg-gradient-to-r from-indigo-600 to-blue-600 text-white border-indigo-700 shadow-lg" : "text-gray-700 bg-gray-50 border-gray-200 hover:bg-gray-100 dark:text-gray-300 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700")}>
        ...
        <div className="space-y-2.5">
          ...
          <select className="w-full bg-gray-50 dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 text-sm md:text-base rounded-xl px-3 md:px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all font-medium" value={filters.tankSize || ''} onChange={(e) => setFilters({ ...filters, tankSize: e.target.value === '' ? null : Number(e.target.value) })}>
        ...
        <div className="space-y-2.5 relative">
          ...
          <input type="text" value={filters.biotope} onChange={(e) => handleBiotopeInput(e.target.value)} onFocus={() => filters.biotope.length >= 2 && setShowSuggestions(true)} onBlur={() => setTimeout(() => setShowSuggestions(false), 200)} placeholder="e.g. 'Amazon'..." className="w-full bg-gray-50 dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 text-sm md:text-base rounded-xl px-3 md:px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all" />
          {showSuggestions && biotopeSuggestions.length > 0 && (
            <div className="absolute z-50 w-full bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl mt-1 max-h-60 overflow-y-auto">
              {biotopeSuggestions.map((b) => (
                <button key={b.id} onClick={() => selectBiotope(b)} className="w-full text-left px-4 py-3 hover:bg-indigo-50 dark:hover:bg-gray-700 transition-colors border-b border-gray-100 dark:border-gray-700 last:border-0 group">
          ...
          <div className="flex flex-wrap gap-1.5 md:gap-2 pt-1">
            {['Blackwater', 'Amazon', 'Rice Paddies'].map(tag => (
              <button key={tag} onClick={() => setFilters({ ...filters, biotope: tag })} className="px-2.5 py-1.5 bg-gray-100 dark:bg-gray-800 hover:bg-indigo-100 dark:hover:bg-indigo-900/30 text-gray-600 dark:text-gray-300 hover:text-indigo-700 dark:hover:text-indigo-400 text-[10px] md:text-[11px] font-bold uppercase rounded-lg border border-gray-200 dark:border-gray-700 transition-all whitespace-nowrap">
                {tag}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-2.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            <FilterChip label="Fish"   icon={<Fish className="w-3 h-3 md:w-3.5 md:h-3.5" />}     isActive={filters.type === 'fish'}        onClick={() => setFilters({ ...filters, type: filters.type === 'fish' ? null : 'fish' })}               className="justify-center" />
            <FilterChip label="Inverts" icon={<PawPrint className="w-3 h-3 md:w-3.5 md:h-3.5" />} isActive={filters.type === 'invertebrate'} onClick={() => setFilters({ ...filters, type: filters.type === 'invertebrate' ? null : 'invertebrate' })} className="justify-center" />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-5 pt-5 border-t-2 border-gray-200 dark:border-gray-700">
        ...
        <div className="bg-gradient-to-br from-rose-50 to-orange-50 dark:from-rose-950/30 dark:to-orange-950/30 p-4 rounded-xl border-2 border-rose-200 dark:border-rose-800">
          <label className="text-xs font-bold text-rose-900 dark:text-rose-300 mb-3 block flex items-center"><Thermometer className="w-4 h-4 mr-2" /> Temperature</label>
          <Slider min={15} max={35} step={0.5} value={[filters.tempMin, filters.tempMax]} onChange={([tempMin, tempMax]) => setFilters({ ...filters, tempMin, tempMax })} formatLabel={(v) => `${v}\u00b0C`} />
        </div>
        ...
        <div className="bg-gradient-to-br from-cyan-50 to-blue-50 dark:from-cyan-950/30 dark:to-blue-950/30 p-4 rounded-xl border-2 border-cyan-200 dark:border-cyan-800">
          <label className="text-xs font-bold text-cyan-900 dark:text-cyan-300 mb-3 block flex items-center"><TestTube className="w-4 h-4 mr-2" /> pH Level</label>
        ...
        <div className="bg-gradient-to-br from-amber-50 to-yellow-50 dark:from-amber-950/30 dark:to-yellow-950/30 p-4 rounded-xl border-2 border-amber-200 dark:border-amber-800">
          <label className="text-xs font-bold text-amber-900 dark:text-amber-300 mb-3 block flex items-center"><Ruler className="w-4 h-4 mr-2" /> Max Body Size: {filters.maxBodySize}cm</label>
          <input type="range" min="2" max="100" step="1" value={filters.maxBodySize} onChange={(e) => setFilters({ ...filters, maxBodySize: Number(e.target.value) })} className="w-full h-2 bg-amber-200 dark:bg-amber-800/30 rounded-lg appearance-none cursor-pointer accent-amber-600" />
        </div>

        <div className="space-y-2.5">
          <label className="text-xs font-bold text-gray-700 dark:text-gray-300 flex items-center"><Activity className="w-4 h-4 mr-2" /> Diet</label>
          <select value={filters.diet} onChange={(e) => setFilters({ ...filters, diet: e.target.value as any })} className="w-full px-4 py-3 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-xl text-sm font-medium focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
        ...
        <div className="space-y-2.5">
          <label className="text-xs font-bold text-gray-700 dark:text-gray-300 flex items-center"><Activity className="w-4 h-4 mr-2" /> Temperament</label>
          <select value={filters.temperament} onChange={(e) => setFilters({ ...filters, temperament: e.target.value as any })} className="w-full px-4 py-3 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-xl text-sm font-medium focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
        ...
        <div className="space-y-2.5">
          <label className="text-xs font-bold text-gray-700 dark:text-gray-300 flex items-center"><Activity className="w-4 h-4 mr-2" /> Behavior Tags</label>
  ...
  return (
    <PageTransition>
      <div className="min-h-screen bg-white dark:bg-slate-900 pb-16 md:pb-20">
        <SEOHead title="Species Database - AquaGuide" description="Browse our complete collection of aquarium fish and invertebrates." />

        <div className="relative h-[45vh] min-h-[400px] md:h-[55vh] md:min-h-[500px] overflow-hidden">
          <div className="absolute inset-0">
            <img src={HEADER_IMAGE_URL} alt="Aquarium species" className="w-full h-full object-cover" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
            ...
            <aside className="hidden lg:block lg:col-span-1">
              <div className="bg-white dark:bg-gray-800 p-5 md:p-6 rounded-xl md:rounded-2xl shadow-lg border-2 border-gray-200 dark:border-gray-700 sticky top-24">
                <div className="flex items-center justify-between mb-5 pb-4 border-b-2 border-gray-200 dark:border-gray-700">
                  ...
                  {hasActiveFilters && <button onClick={resetFilters} className="text-xs text-rose-600 dark:text-rose-400 font-bold hover:underline flex items-center gap-1"><X className="w-3 h-3" /> Reset</button>}
                </div>
            ...
            <MobileFilterDrawer isOpen={isMobileDrawerOpen} onClose={() => setIsMobileDrawerOpen(false)}>
              <div className="flex items-center justify-between mb-6 pb-4 border-b-2 border-gray-200">
                ...
                {hasActiveFilters && <button onClick={resetFilters} className="text-xs text-rose-600 font-bold hover:underline flex items-center gap-1"><X className="w-3 h-3" /> Reset</button>}
              </div>
              <FilterContent />
            </MobileFilterDrawer>
              ...
              {filteredSpecies.length > 0 ? (
                <>
                  <Suspense fallback={<SpeciesGridSkeleton />}>
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.2 }} className="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-5">
                      {displayedSpecies.map((s: Species, index: number) => (
                        <motion.div key={s.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: Math.min(index * 0.03, 0.5) }}>
                          <SpeciesCard data={s} />
                        </motion.div>
                      ))}
                    </motion.div>
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }} className="text-center py-12 md:py-20 bg-white dark:bg-gray-800 rounded-2xl md:rounded-3xl border-2 border-dashed border-gray-300 dark:border-gray-700 shadow-lg">
                  ...
                  <p className="text-sm md:text-base text-gray-500 dark:text-gray-400 max-w-md mx-auto mb-5 md:mb-6 px-4">Try adjusting your filters or search term.</p>
                  <button onClick={resetFilters} className="px-5 md:px-8 py-2.5 md:py-4 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white text-sm md:text-base font-bold rounded-xl transition-all shadow-lg hover:shadow-xl transform hover:scale-105">
                    Clear All Filters
                  </button>
                </motion.div>
              )}
...
const FilterChip = ({ label, icon, isActive, onClick, colorClass = 'text-gray-700 bg-gray-50 border-gray-200 hover:bg-gray-100 dark:text-gray-300 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700', activeClass = 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white border-indigo-700 shadow-lg', className = '' }: any) => (
  <button onClick={onClick} className={cn('px-3 py-2 md:py-2.5 rounded-lg md:rounded-xl text-[11px] md:text-xs font-bold uppercase tracking-wide border-2 transition-all duration-200 flex items-center gap-1.5 md:gap-2', isActive ? activeClass : colorClass, className)}>
    {icon}
