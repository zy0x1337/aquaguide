...
export const TankBuilderPage = () => {
  ...
  const updateCustomTank = () => {
    ...
    setTankConfig({ ...PRESET_TANKS[7], ...customDimensions, volume: Math.round(volume), aspectRatio });
  };
  ...
  const calculateDisplaySize = (realSizeCM: number, itemType: 'fish' | 'plant' | 'hardscape'): number => {
    const tankLengthCM = tankConfig.length;
    const sizeRatio = realSizeCM / tankLengthCM;
    const baseSize = sizeRatio * 800;
    
    if (itemType === 'fish') {
      return Math.max(30, Math.min(100, baseSize));
    } else if (itemType === 'plant') {
      const heightRatio = realSizeCM / tankConfig.height;
      return Math.max(35, Math.min(120, heightRatio * 600));
    } else {
      return Math.max(25, Math.min(80, baseSize));
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 -mt-8"><div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-6"><h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center"><Layers className="w-5 h-5 mr-2 text-emerald-600" /> Tank Stats</h3><div className="space-y-4"><StatRow label="💧 Volume" value={`${tankConfig.volume}L`} /><StatRow label="📐 Dimensions" value={`${tankConfig.length}×${tankConfig.width}×${tankConfig.height}cm`} /><StatRow label="🐟 Total Fish" value={totalFish.toString()} /><StatRow label="🌿 Plants" value={totalPlants.toString()} /><StatRow label="🪨 Hardscape" value={totalHardscape.toString()} />
                ...
                {phRange && (<div className="flex items-center justify-between"><div className="flex items-center gap-2 text-sm text-slate-600"><Droplets className="w-4 h-4" /><span>pH Range</span></div><span className="font-bold text-sm text-slate-900">{phRange}</span></div>)}</n              </div></div>
...
const Tank3DView = ({ items, tankConfig, showGrid, calculateDisplaySize, onRemoveItem, onToggleLock, onUpdatePosition, onUpdateCount, draggedItem, setDraggedItem, selectedItem, setSelectedItem }: { items: TankItem[]; tankConfig: TankConfig; showGrid: boolean; calculateDisplaySize: (size: number, type: 'fish' | 'plant' | 'hardscape') => number; onRemoveItem: (id: string) => void; onToggleLock: (id: string) => void; onUpdatePosition: (id: string, x: number, y: number) => void; onUpdateCount: (id: string, delta: number) => void; draggedItem: string | null; setDraggedItem: (id: string | null) => void; selectedItem: string | null; setSelectedItem: (id: string | null) => void; }) => {
