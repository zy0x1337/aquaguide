...
const ComparisonPage = () => {
  ...
  const comparisonData = [
    ...
    { label: 'Difficulty', icon: <Activity className="w-4 h-4" />, values: comparedSpecies.map(s => s.care.difficulty), category: 'care', highlightDiff: false, colorCode: (v: string) => v === 'beginner' ? 'text-emerald-700 bg-emerald-50 border-emerald-200 dark:text-emerald-400 dark:bg-emerald-950/30 dark:border-emerald-800' : v === 'medium' ? 'text-amber-700 bg-amber-50 border-amber-200 dark:text-amber-400 dark:bg-amber-950/30 dark:border-amber-800' : 'text-rose-700 bg-rose-50 border-rose-200 dark:text-rose-400 dark:bg-rose-950/30 dark:border-rose-800' },
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 -mt-4 mb-4">
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-800 p-4 flex items-center gap-3">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">Show only differences</span>
          </label>
          {showOnlyDifferences && (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                  <th className="text-left p-4 font-bold text-gray-700 dark:text-gray-300 w-48 sticky left-0 bg-gray-50 dark:bg-gray-800 z-10">Property</th>
                  {comparedSpecies.map((species) => (
                    <th key={species.id} className="p-4 text-center relative min-w-[200px]">
                      <button
                        ...
                        className="absolute top-2 right-2 p-1 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full transition-colors z-10"
                        aria-label="Remove"
                      >
                        <X className="w-4 h-4 text-gray-500 dark:text-gray-400" />
                      </button>
                      <div className="font-bold text-gray-900 dark:text-white mb-1 pr-8">{species.taxonomy.commonName}</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400 italic">{species.taxonomy.scientificName}</div>
                    </th>
                  ))}
                </tr>
              </thead>
          ...
          <div className="lg:hidden">
            {/* Species Headers */}
            <div className="border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800 p-4">
              <h3 className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-3">Comparing Species:</h3>
              <div className="space-y-2">
                {comparedSpecies.map((species) => (
                  <div key={species.id} className="flex items-center justify-between bg-white dark:bg-gray-900 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
                    <div className="flex-1">
                      <div className="font-bold text-gray-900 dark:text-white text-sm">{species.taxonomy.commonName}</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400 italic">{species.taxonomy.scientificName}</div>
                    </div>
                    <button
                      onClick={() => removeFromComparison(species.id)}
...
const ComparisonRow = ({ label, icon, values, highlightDiff, colorCode }: { 
  ...
  return (
    <tr className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
      <td className="p-4 font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-2 sticky left-0 bg-white dark:bg-gray-900 z-10">
      ...
      {values.map((value, idx) => (
        <td key={idx} className="p-4 text-center">
          <span className={`inline-block px-3 py-1.5 rounded-lg font-semibold border ${
            ...
            shouldHighlight ? 'bg-amber-50 text-amber-900 border-amber-200 dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-800' : 
  ...
  return (
    <div className="bg-white dark:bg-gray-900">
      <button
        ...
        className="w-full flex items-center justify-between p-4 font-bold text-gray-900 dark:text-white hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
      >
              ...
              {data.map((row, idx) => (
                <div key={idx} className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
                  <div className="flex items-center gap-2 mb-3 text-sm font-bold text-gray-700 dark:text-gray-300">
                    <span className="text-indigo-600 dark:text-indigo-400">{row.icon}</span>
                    {row.label}
                  </div>
                  <div className="space-y-2">
                    {species.map((s, sIdx) => {
                      ...
                      return (
                        <div key={s.id} className="flex items-center justify-between">
                          <span className="text-xs font-semibold text-gray-600 dark:text-gray-400 truncate max-w-[140px]">{s.taxonomy.commonName}</span>
                          <span className={`inline-block px-2.5 py-1 rounded-lg text-xs font-bold border ${
                            row.colorCode ? row.colorCode(value) :
                            isDifferent ? 'bg-amber-50 text-amber-900 border-amber-200 dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-800' :
                            'bg-white text-gray-800 border-gray-200 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700'
                          }`}>
                            {value}
                          </span>
                        </div>
...
const CompatibilityWarning = ({ species }: { species: Species[] }) => {
  ...
  if (warnings.length === 0) {
    return (
      <div className="mt-6 bg-emerald-50 dark:bg-emerald-950/30 border-2 border-emerald-200 dark:border-emerald-800 rounded-xl p-5">
        <div className="flex gap-3 mb-3">
          <div className="w-6 h-6 bg-emerald-500 rounded-full flex items-center justify-center flex-shrink-0">
            <span className="text-white text-sm font-bold">✓</span>
          </div>
          <div className="flex-1">
            <p className="font-bold text-emerald-900 dark:text-emerald-300 text-lg">Excellent Compatibility!</p>
            <p className="text-sm text-emerald-700 dark:text-emerald-400 mt-1">These species are compatible for keeping together.</p>
          </div>
        </div>
        <div className="ml-9 space-y-1.5">
          {positives.map((positive, idx) => (
            <div key={idx} className="flex items-start gap-2 text-sm text-emerald-800 dark:text-emerald-300">
  ...
  return (
    <div className="mt-6 space-y-4">
      {warnings.length > 0 && (
        <div className="bg-amber-50 dark:bg-amber-950/30 border-2 border-amber-300 dark:border-amber-800 rounded-xl p-5">
          <div className="flex gap-3 mb-3">
            <AlertTriangle className="w-6 h-6 text-amber-600 dark:text-amber-400 flex-shrink-0" />
            <div className="flex-1">
              <p className="font-bold text-amber-900 dark:text-amber-300 text-lg">Compatibility Warnings</p>
              <p className="text-sm text-amber-700 dark:text-amber-400 mt-1">These species may have compatibility issues:</p>
            </div>
          </div>
      ...
      {positives.length > 0 && (
        <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-xl p-5">
          <p className="font-bold text-blue-900 dark:text-blue-300 mb-2">Compatible Parameters:</p>
...
const EmptyState = () => (
  <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950 px-6">
    <div className="text-center max-w-md mx-auto bg-white dark:bg-gray-900 rounded-2xl p-12 shadow-lg border border-gray-200 dark:border-gray-800">
      <div className="w-20 h-20 bg-indigo-100 dark:bg-indigo-950/30 rounded-2xl mx-auto mb-6 flex items-center justify-center">
        <AlertTriangle className="w-10 h-10 text-indigo-600 dark:text-indigo-400" />
      </div>
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">No Species Selected</h1>
      <p className="text-gray-600 dark:text-gray-400 mb-6">Add at least 2 species to compare them side-by-side.</p>
      <Link to="/species" className="inline-block px-6 py-3 bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 text-white font-bold rounded-xl transition-colors shadow-lg">
        Browse Species
      </Link>
    </div>
  </div>
