...
const ProfilePage = () => {
  ...
  // The target user ID – works for both own profile and public guest view
  ...
  // Always pass targetUserId so guests can read a public profile's favorites
  ...
  // Always fetch details regardless of auth state – guests need this for public profiles
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-16">
        ...
        <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800">
          <div className="max-w-5xl mx-auto">
            ...
            {/* ── Cover image ── */}
            <div className="relative h-44 sm:h-56 overflow-hidden rounded-b-none">
              ...
              {/* Scrim: always-on subtle gradient at bottom so avatar+text stay readable */}
              <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/40 to-transparent pointer-events-none" />

              {/* Upload button – only visible on own profile, hover */}
              {isOwnProfile && (
                <button
            ...
            {/* ── Profile strip (avatar + info + actions) ── */}
            <div className="px-4 sm:px-6 lg:px-8 pt-0 pb-0">
              ...
              <div className="flex items-end justify-between -mt-10 sm:-mt-12 mb-3">
                ...
                <div className="relative group z-20 flex-shrink-0">
                  ...
                  {/* Ring wrapper for clean separation from cover */}
                  <div className="p-1 bg-white dark:bg-gray-900 rounded-2xl shadow-lg">
                    {avatarUrl
                      ? <img
                          src={avatarUrl}
                          alt="Avatar"
                          className="w-20 h-20 sm:w-24 sm:h-24 rounded-xl object-cover block"
                        />
                      : <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-xl bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-white text-xl font-black">
                          {getUserInitials()}
                        </div>
                    }
                  </div>
                ...
                <div className="flex items-center gap-2 pb-1">
                  ...
                  {isOwnProfile && (
                    <button
                      onClick={() => { if (isEditing) handleSave(); setIsEditing(e => !e); }}
              ...
              <div className="mb-4">
                ...
                {profile.bio && (
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-2 leading-relaxed max-w-2xl">
                    {profile.bio}
                  </p>
                )}
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'overview' && (
              <motion.div key="overview" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} className="space-y-5">
                ...
                {(featuredTanks.length > 0 || tanksLoading) && (
                  <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {isOwnProfile && (
                        <Link to="/my-tanks" className="text-xs font-bold text-cyan-600 dark:text-cyan-400 hover:underline">
                          View All →
                        </Link>
                      )}
                    </div>
                            ...
                            >
                              ...
                              <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
                                {(tank.inhabitants?.fish?.length ?? 0) > 0 && (
                                  <span className="flex items-center gap-1"><Fish className="w-3 h-3" />{tank.inhabitants!.fish.reduce((s,i)=>s+i.quantity,0)}</span>
                                )}
                                {(tank.inhabitants?.plants?.length ?? 0) > 0 && (
                                  <span className="flex items-center gap-1"><Leaf className="w-3 h-3" />{tank.inhabitants!.plants.reduce((s,i)=>s+i.quantity,0)}</span>
                                )}
                                {tank.parameters?.ph && (
                                  <span className="flex items-center gap-1"><Droplets className="w-3 h-3" />pH {tank.parameters.ph}</span>
                                )}
                              </div>
                            </Link>
                          ))}
                ...
                {/* 2-col grid: Recent Favorites + Achievements preview */}
                            ...
                            <div className="w-9 h-9 rounded-lg bg-gray-100 dark:bg-gray-800 overflow-hidden flex-shrink-0">
                              {data.image_url
                                ...
                                : <div className="w-full h-full flex items-center justify-center">
                                    {fav.item_type === 'species' ? <Fish className="w-4 h-4 text-gray-400" /> : <Leaf className="w-4 h-4 text-gray-400" />}
                                  </div>
                              }
                            </div>
                            <div className="flex-1 min-w-0">
                  ...
                  >
                    ...
                    <div className="flex gap-2 mt-4">
                      <button onClick={handleSave}
                        className="flex items-center gap-1.5 px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white font-bold rounded-xl text-sm transition-all">
                        <Save className="w-4 h-4" />Save Changes
                      </button>
                      <button onClick={() => setIsEditing(false)}
                        className="px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 font-bold rounded-xl text-sm transition-all">
                        Cancel
                      </button>
                    </div>
                  </motion.div>
                ...
                {/* Beta feedback – own profile only */}
                {isOwnProfile && (
                  <div className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/20 dark:to-purple-950/20 rounded-2xl border border-indigo-100 dark:border-indigo-900/50 p-5">
                    ...
                    <div className="flex gap-2">
                      <textarea
                        ...
                        placeholder="I really like… but maybe you could add…"
                        rows={2}
                  ...
                  <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="font-black text-gray-900 dark:text-white text-sm">
                        Fish <span className="text-gray-400 font-normal">({favSpecies.length})</span>
                      </h3>
                    </div>
                        ...
                        return (
                          <div key={fav.id} className="relative group">
                            ...
                            {isOwnProfile && isEditing && (
                              <button
                                onClick={e => { e.preventDefault(); toggleFavorite('species', fav.item_slug); }}
                ...
                {favPlants.length > 0 && (
                  <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="font-black text-gray-900 dark:text-white text-sm">
                        Plants <span className="text-gray-400 font-normal">({favPlants.length})</span>
                      </h3>
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                      {favPlants.map(fav => {
                        ...
                        return (
                          <div key={fav.id} className="relative group">
                            ...
                            {isOwnProfile && isEditing && (
                              <button
                                onClick={e => { e.preventDefault(); toggleFavorite('plant', fav.item_slug); }}
                                className="absolute top-2 right-2 p-1.5 bg-red-500 hover:bg-red-600 text-white rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-all"
                              >
                                <Trash2 className="w-3 h-3" strokeWidth={2.5} />
                              </button>
              ...
              <motion.div key="achievements" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} className="space-y-5">
                <div className="bg-gradient-to-r from-amber-50 to-yellow-50 dark:from-amber-950/20 dark:to-yellow-950/20 border border-amber-200 dark:border-amber-800/50 rounded-2xl p-5">
                  ...
                  <div className="mt-3 w-full bg-amber-200/60 dark:bg-amber-900/30 rounded-full h-2 overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${Math.round((unlockedCount / achievements.length) * 100)}%` }}
                      transition={{ duration: 0.8, ease: 'easeOut' }}
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${
                        achFilter === cat
                          ...
                          : 'bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-cyan-300'
                      }`}
                    >
