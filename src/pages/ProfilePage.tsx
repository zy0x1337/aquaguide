...
const LevelBadge = ({ info, size = 'md' }: { info: LevelInfo; size?: 'sm' | 'md' | 'lg' }) => {
  ...
  const cls = colorMap[info.color] ?? colorMap.slate;
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border font-bold ${sizes[size]} ${cls}`}>
...
const XPBar = ({ info }: { info: LevelInfo }) => {
  const colorBarMap: Record<string, string> = {
    slate: 'bg-slate-500', blue: 'bg-blue-500', cyan: 'bg-cyan-500',
    teal: 'bg-teal-500', emerald: 'bg-emerald-500', green: 'bg-green-500',
    indigo: 'bg-indigo-500', violet: 'bg-violet-500', purple: 'bg-purple-500',
    amber: 'bg-amber-500', rose: 'bg-rose-500',
  };
  const bar = colorBarMap[info.color] ?? 'bg-slate-500';
  return (
    <div className="mt-2">
      <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 mb-1">
        <span className="font-semibold">{info.xp} XP</span>
        <span>{info.xpForNextLevel} XP needed for Lv.{info.level + 1}</span>
      </div>
...
const ProfilePage = () => {
  ...
  const { favorites, toggleFavorite } = useFavorites(userId || user?.id);
  ...
  useEffect(() => {
    ...
    getFeaturedTanksForUser(id).then(setFeaturedTanks).catch(console.error).finally(() => setTanksLoading(false));
  ...
  const loadProfile = async () => {
    ...
    try {
      const { data: pd } = await supabase
        ...
        .eq('id', id).single();
      const { data: ud } = await supabase.auth.admin.getUserById(id);
      const { count: tc } = await supabase.from('tanks').select('*', { count: 'exact', head: true }).eq('user_id', id);
      const { count: pc } = await supabase.from('tanks').select('*', { count: 'exact', head: true }).eq('user_id', id).eq('is_public', true);
  ...
  const handleShareProfile = async () => {
    await copyToClipboard(`${window.location.origin}/profile/${userId || user?.id}`);
    setLinkCopied(true);
        ...
        <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="relative h-44 sm:h-56 group overflow-hidden">
              ...
              {isOwnProfile && (
                <button onClick={() => headerInputRef.current?.click()} disabled={uploadingHeader}
                  className="absolute bottom-3 right-3 z-10 p-2 bg-black/40 hover:bg-black/60 backdrop-blur-sm rounded-xl text-white transition-all opacity-0 group-hover:opacity-100">
                  {uploadingHeader ? <Upload className="w-4 h-4 animate-pulse" /> : <Camera className="w-4 h-4" />}
                </button>
              )}
            </div>
            ...
            <div className="px-4 sm:px-6 lg:px-8">
              <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4 -mt-14 sm:-mt-16 pb-4">

                {/* Avatar */}
                <div className="relative group z-20 flex-shrink-0">
                  <input ref={fileInputRef} type="file" accept="image/*" onChange={handleAvatarUpload} className="hidden" />
                  {avatarUrl
                    ? <img src={avatarUrl} alt="Avatar" className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl shadow-xl border-4 border-white dark:border-gray-900 object-cover" />
                    : <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl bg-gradient-to-br from-coral-500 to-sapphire-500 flex items-center justify-center text-white text-2xl font-black shadow-xl border-4 border-white dark:border-gray-900">{getUserInitials()}</div>
                  }
                  {isOwnProfile && (
                    <button onClick={() => fileInputRef.current?.click()} disabled={uploading}
                      ...
                      {uploading ? <Upload className="w-6 h-6 text-white animate-pulse" /> : <Camera className="w-6 h-6 text-white" />}
                    </button>
                  )}
                </div>
                ...
                <div className="flex-1 min-w-0 pt-2">
                  ...
                  {profile.bio && <p className="text-sm text-gray-600 dark:text-gray-400 mb-2 line-clamp-2">{profile.bio}</p>}
                  <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
                    {profile.location && <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />{profile.location}</span>}
                    {profile.website && (
                      <a href={profile.website} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-1 text-coral-600 dark:text-coral-400 hover:underline">
                ...
                <div className="flex items-center gap-2 pb-1">
                  ...
                  {isOwnProfile && (
                    <button onClick={() => { if (isEditing) handleSave(); setIsEditing(e => !e); }}
                      className="flex items-center gap-1.5 px-3 py-2 bg-coral-600 hover:bg-coral-700 text-white font-bold rounded-xl transition-all text-sm">
                  ...
                  return (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id as TabType)}
                      className={`relative flex items-center gap-1.5 px-4 py-3 text-sm font-bold whitespace-nowrap transition-colors ${
                        active ? 'text-coral-600 dark:text-coral-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'
                      }`}>
                      <Icon className="w-4 h-4" strokeWidth={2.5} />
                      {tab.label}
                      {'badge' in tab && (tab as any).badge > 0 && (
                        <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${
                          active ? 'bg-coral-100 dark:bg-coral-900/30 text-coral-700 dark:text-coral-300' : 'bg-gray-100 dark:bg-gray-800 text-gray-500'
                ...
                {isOwnProfile && profileCompletion < 100 && (
                  <div className="bg-gradient-to-r from-indigo-50 to-violet-50 dark:from-indigo-950/20 dark:to-violet-950/20 border border-indigo-100 dark:border-indigo-900/50 rounded-2xl p-4">
                    ...
                    <div className="w-full bg-indigo-100 dark:bg-indigo-900/40 rounded-full h-2 overflow-hidden mb-3">
                      <motion.div initial={{ width: 0 }} animate={{ width: `${profileCompletion}%` }} transition={{ duration: 0.7 }}
                ...
                {(featuredTanks.length > 0 || tanksLoading) && (
                  <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {isOwnProfile && <Link to="/my-tanks" className="text-xs font-bold text-coral-600 dark:text-coral-400 hover:underline">View All →</Link>}
                    </div>
                    {tanksLoading
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button onClick={() => setActiveTab('favorites')} className="text-xs font-bold text-coral-600 dark:text-coral-400 hover:underline">View All</button>
                    </div>
                      ...
                      {favorites.slice(0, 4).map(fav => {
                        ...
                        return (
                          <Link key={fav.id} to={`/${fav.item_type === 'species' ? 'species' : 'plants'}/${data.slug}`}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="text-xs font-bold text-gray-900 dark:text-white truncate group-hover:text-coral-600 dark:group-hover:text-coral-400 transition-colors">{data.common_name}</div>
                  ...
                  <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button onClick={() => setActiveTab('achievements')} className="text-xs font-bold text-coral-600 dark:text-coral-400 hover:underline">View All</button>
                    </div>
                      ...
                      {achievements.slice(0, 8).map(a => {
                        ...
                        return (
                          <div key={a.id} title={a.name}
                            ...
                            {a.unlocked
                              ? <Icon className="w-5 h-5 text-amber-600 dark:text-amber-400" strokeWidth={2} />
                  ...
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <input type="text" value={profile.displayName} onChange={e => setProfile({...profile, displayName: e.target.value})}
                          className="w-full px-3 py-2.5 bg-gray-50 dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-coral-500 focus:border-coral-500 outline-none transition-all text-sm" />
                      </div>
                      <div>
                        ...
                        <textarea value={profile.bio} onChange={e => setProfile({...profile, bio: e.target.value})} rows={3}
                          ...
                          className="w-full px-3 py-2.5 bg-gray-50 dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-coral-500 focus:border-coral-500 outline-none transition-all resize-none text-sm" />
                      </div>
                    ...
                    <div className="flex gap-2 mt-4">
                      <button onClick={handleSave}
                        className="flex items-center gap-1.5 px-4 py-2 bg-coral-600 hover:bg-coral-700 text-white font-bold rounded-xl text-sm transition-all">
                        <Save className="w-4 h-4" />Save Changes
                      </button>
                      <button onClick={() => setIsEditing(false)}
                        className="px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 font-bold rounded-xl text-sm transition-all">Cancel
                      </button>
                    </div>
                  </motion.div>
                )}
                ...
                {favSpecies.length > 0 && (
                  <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
                    <div className="flex items-center gap-2 mb-4">
                      <Fish className="w-4 h-4 text-coral-500" strokeWidth={2.5} />
                  ...
                  {achievementCategories.map(cat => (
                    <button key={cat} onClick={() => setAchFilter(cat)}
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${
                        achFilter === cat ? 'bg-coral-600 text-white shadow-md' : 'bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-coral-300'
                      }`}>
                      {cat.charAt(0).toUpperCase() + cat.slice(1)}
                      {cat !== 'all' && (
                        <span className="ml-1 opacity-70">({achievements.filter(a => a.category === cat && a.unlocked).length}/{achievements.filter(a => a.category === cat).length})</span>
                      )}
                    </button>
                  ))}
                </div>
