...
import { ArrowLeft, Plus, Droplets, Thermometer, Fish as FishIcon, Leaf, Trash2, AlertTriangle, CheckCircle, Edit, Activity, Wrench, Mountain, Lightbulb, Bell, Sparkles, Hammer } from 'lucide-react';
...
const TankDetailPage = () => {
  ...
  const handleEditInBuilder = () => {
    ...
    // Estimate dimensions from volume (2×1×1 ratio)
    ...
    const tankConfig: TankConfig = {
      ...
      substrate:   (tank.substrate as any) || 'gravel',
      hasFilter:   false,
      hasHeater:   false,
    };
    ...
    // Restore fish
    (tank.inhabitants?.fish || []).forEach((inhabitant, idx) => {
      const species = allSpecies.find(s => s.id === inhabitant.speciesId);
      if (!species) return;
      items.push({
        id: `fish-${inhabitant.speciesId}-${now}-${idx}`,
        type: 'fish',
        data: species,
        position: { x: 20 + Math.random() * 60, y: 20 + Math.random() * 60, z: 40 + Math.random() * 30 },
        count: inhabitant.quantity,
        locked: false,
        notes: '',
    ...
    // Restore plants
    (tank.inhabitants?.plants || []).forEach((inhabitant, idx) => {
      const plant = allPlants.find(p => p.id === inhabitant.speciesId);
      if (!plant) return;
      items.push({
        id: `plant-${inhabitant.speciesId}-${now}-${idx}`,
        type: 'plant',
        data: plant,
    ...
    localStorage.setItem(
      ...
      JSON.stringify({ tankConfig, items, customDimensions: { length, width, height }, timestamp: now })
    );
  ...
  const handleEditTank = async (updatedTank: Tank) => {
    ...
    try {
      const updated = await updateTank(id, {
        name: updatedTank.name, type: updatedTank.type,
        volumeLiters: updatedTank.volumeLiters, substrate: updatedTank.substrate, lighting: updatedTank.lighting,
      });
      setTank(updated);
  ...
  const handleAddParameterReading = async (reading: any) => {
    ...
    try {
      ...
      await loadTank(); await loadHistory();
  ...
  const handleDeleteParameterReading = async (readingId: string) => {
    try {
      await deleteParameterReading(readingId); await loadHistory();
  ...
  const handleAddMaintenanceLog = async (log: any) => {
    if (!id) return;
    try {
      await addMaintenanceLog(id, log); await loadHistory();
  ...
  const handleDeleteMaintenanceLog = async (logId: string) => {
    try {
      await deleteMaintenanceLog(logId); await loadHistory();
  ...
  fishSpecies.forEach((species, i) => {
    fishSpecies.slice(i + 1).forEach(other => {
      if (species && other) {
        const bad = species.behavior.compatibility.badMates.some(bm =>
        ...
        if (bad) compatibilityWarnings.push(`${species.taxonomy.commonName} and ${other.taxonomy.commonName} may not be compatible`);
      }
    });
  });
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-teal-400/20 hover:bg-teal-400/30 text-white border border-teal-300/40 hover:border-teal-300/70 px-5 py-3 rounded-xl font-semibold transition-all w-fit"
                title="Open this tank in the Tank Builder to redesign it"
              >
...
const OverviewTab = ({ tank, compatibilityWarnings, onAddFish, onAddPlant, onRemoveInhabitant }: any) => {
  const substrateLabel = (s?: string) =>
    ({ sand: 'Sand', gravel: 'Gravel', soil: 'Aqua Soil', bare: 'Bare Bottom' } as any)[s || ''] || 'Not specified';
  const lightingLabel = (l?: string) =>
    ({ low: 'Low (10–30 PAR)', medium: 'Medium (30–50 PAR)', high: 'High (50+ PAR)' } as any)[l || ''] || 'Not specified';
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="space-y-6 md:space-y-8">
      {compatibilityWarnings.length > 0 && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          className="bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-500 rounded-lg p-5 md:p-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
        className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-5 md:p-6">
        <h2 className="text-xl md:text-2xl font-bold text-slate-900 dark:text-white mb-5 md:mb-6 flex items-center gap-2">
          <Mountain className="w-6 h-6 text-indigo-600" />Tank Setup
        </h2>
        <div className="grid sm:grid-cols-2 gap-4">
          <CleanSetupCard icon={<Mountain className="w-6 h-6 text-amber-600" />} label="Substrate" value={substrateLabel(tank.substrate)} isEmpty={!tank.substrate} />
          <CleanSetupCard icon={<Lightbulb className="w-6 h-6 text-yellow-600" />} label="Lighting"  value={lightingLabel(tank.lighting)}  isEmpty={!tank.lighting} />
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-5 md:p-6">
        <h2 className="text-xl md:text-2xl font-bold text-slate-900 dark:text-white mb-5 md:mb-6 flex items-center gap-2">
          <Activity className="w-6 h-6 text-blue-600" />Current Water Parameters
        </h2>
      ...
      <InhabitantsSection title="Fish"   icon={<FishIcon className="w-6 h-6 text-indigo-600" />} inhabitants={tank.inhabitants?.fish   || []} speciesData={allSpecies} linkPrefix="/species" onAdd={onAddFish}  onRemove={(id: string) => onRemoveInhabitant(id, 'fish')}  emptyMessage="No fish added yet"   addButtonLabel="Add Fish" />
      <InhabitantsSection title="Plants" icon={<Leaf     className="w-6 h-6 text-emerald-600" />} inhabitants={tank.inhabitants?.plants || []} speciesData={allPlants}  linkPrefix="/plants"  onAdd={onAddPlant} onRemove={(id: string) => onRemoveInhabitant(id, 'plant')} emptyMessage="No plants added yet" addButtonLabel="Add Plant" />
    </motion.div>
  );
};
  ...
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="space-y-6">
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <h2 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
        <Activity className="w-6 h-6 md:w-7 md:h-7 text-blue-600" />Parameter History
      </h2>
...
const MaintenanceTab = ({ logs, onAddLog, onDeleteLog }: any) => (
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="space-y-6">
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <h2 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
        <Wrench className="w-6 h-6 md:w-7 md:h-7 text-purple-600" />Maintenance History
      </h2>
...
const RemindersTab = ({ tankId, tankName }: { tankId: string; tankName: string }) => (
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="space-y-6">
    <h2 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
      <Bell className="w-6 h-6 md:w-7 md:h-7 text-indigo-600" />Reminders
    </h2>
...
const CleanTabButton = ({ active, onClick, icon, label, badge }: any) => (
  <button onClick={onClick}
    className={`relative flex items-center gap-2 px-4 md:px-5 py-3 md:py-4 font-semibold transition-all whitespace-nowrap ${
    ...
    {badge !== undefined && badge > 0 && (
      <span className="ml-1 px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-bold rounded-full">{badge}</span>
    )}
    {active && (
...
const CleanSetupCard = ({ icon, label, value, isEmpty }: any) => (
  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 md:p-5 shadow-sm">
    <div className="flex items-center gap-3 mb-3">
      <div className="w-10 h-10 md:w-12 md:h-12 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center">{icon}</div>
...
const CleanParamCard = ({ label, value, status }: any) => {
  ...
  }[status as 'good' | 'warning' | 'danger'];
  return (
    <div className={`bg-white dark:bg-slate-900 border ${cfg.border} rounded-xl p-4 shadow-sm`}>
  ...
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
    ...
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5 md:mb-6">
      <h2 className="text-xl md:text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
        {icon}{title} ({inhabitants.length} species)
      </h2>
    ...
    {inhabitants.length === 0
      ? <div className="text-center py-12 text-slate-500 dark:text-slate-400"><p>{emptyMessage}</p></div>
      : <div className="space-y-3">
          {inhabitants.map((inh: any, i: number) => {
            const sp = speciesData.find((s: any) => s.id === inh.speciesId);
            if (!sp) return null;
            return (
              <InhabitantCard key={`${inh.speciesId}-${i}`}
                name={inh.speciesName} scientificName={sp.taxonomy.scientificName}
                quantity={inh.quantity} slug={sp.slug} imageUrl={sp.imageUrl}
                linkPrefix={linkPrefix} onRemove={() => onRemove(inh.speciesId)} />
            );
          })}
        </div>
    }
...
const InhabitantCard = ({ name, scientificName, quantity, slug, imageUrl, linkPrefix, onRemove }: any) => (
  <div className="group flex items-center gap-3 md:gap-4 bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl p-3 md:p-4 border border-slate-200 dark:border-slate-700 transition-all">
    ...
    <button onClick={e => { e.preventDefault(); if (confirm(`Remove ${name} from tank?`)) onRemove(); }}
      className="p-2 text-slate-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors flex-shrink-0" title="Remove">
