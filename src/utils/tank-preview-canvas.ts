...
export async function generateTankPreview(
  ...
  if (stocking > 0) {
    ...
    ctx.fillStyle = stockingColor; ctx.fill();
  }
  ...
  uniqueFish.forEach((item, idx) => {
    ...
    ctx.beginPath(); ctx.arc(tagX + 16, tagY + 18, 6, 0, Math.PI * 2);
  ...
  if (fishItems.length > MAX_TAGS) {
    ...
    ctx.fillText(`+${fishItems.length - MAX_TAGS} more`, tagX + 4, tagY + 25);
  }

  // ─── URL watermark (bottom right) ──────────────────────────────────────────────
  const shortUrl = shareUrl.replace(/^https?:\/\//, '').slice(0, 60);
  ctx.font = '20px -apple-system, BlinkMacSystemFont, sans-serif';
  ctx.fillStyle = 'rgba(99,102,241,0.7)'; ctx.textAlign = 'right';
