  ...
  return (
    ...
    >
        ...
        <div className="flex items-start justify-between mb-3 sm:mb-4">
          ...
          <div className="flex gap-1.5 sm:gap-2 ml-2 flex-shrink-0">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="p-2 sm:p-2.5 bg-white dark:bg-slate-700 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 border border-slate-300 dark:border-slate-600 hover:border-indigo-300 dark:hover:border-indigo-700 rounded-lg transition-all shadow-sm hover:shadow"
              title="Edit Tank"
            >
              <Edit className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-600 dark:text-slate-300 group-hover:text-indigo-600 dark:group-hover:text-indigo-400" />
            </motion.button>
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              ...
              className="p-2 sm:p-2.5 bg-white dark:bg-slate-700 hover:bg-red-50 dark:hover:bg-red-900/20 border border-slate-300 dark:border-slate-600 hover:border-red-300 dark:hover:border-red-700 rounded-lg transition-all shadow-sm hover:shadow"
              title="Delete Tank"
            >
              <Trash2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-600 dark:text-slate-300 group-hover:text-red-600 dark:group-hover:text-red-400" />
            </motion.button>
          </div>
        </div>
      ...
      <div className="relative p-4 sm:p-5">
        {/* Water Parameters - More Professional */}
        ...
        {/* Professional Manage Tank Button */}
        <Link
          to={`/my-tanks/${tank.id}`}
          className="group/btn relative w-full flex items-center justify-center gap-2 bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white font-bold px-4 sm:px-5 py-3 sm:py-3.5 rounded-xl transition-all border border-slate-800 dark:border-slate-700 shadow-lg hover:shadow-xl overflow-hidden"
        >
          {/* Subtle shine effect */}
          ...
          <span className="relative z-10 text-sm sm:text-base">Manage Tank</span>
          <ChevronRight className="relative w-4 h-4 sm:w-5 sm:h-5 group-hover/btn:translate-x-1 transition-transform z-10" />
        </Link>
      </div>
    </motion.div>
  );
...
const CleanStatBadge = ({ icon, value, label, color }: { icon: React.ReactNode; value: string | number; label: string; color: string }) => {
  ...
  return (
    <div className={`${colorClasses} border rounded-lg px-2 py-2 text-center`}>
      <div className="flex items-center justify-center gap-1 mb-0.5 sm:mb-1">
        {icon}
        <span className="text-sm sm:text-base font-black">{value}</span>
      </div>
      <div className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider opacity-80">{label}</div>
    </div>
  );
};

// Compact Parameter Card - More Professional
const ParamCard = ({ label, value, status }: { label: string; value: string | number; status: 'good' | 'warning' | 'danger' }) => {
  ...
  return (
    <div className={`${config.bg} border ${config.border} rounded-lg p-2 sm:p-2.5 transition-all hover:shadow-sm relative overflow-hidden`}>
      <div className="absolute top-1.5 sm:top-2 right-1.5 sm:right-2">
        <div className={`w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full ${config.dot} ${status === 'good' ? 'animate-pulse' : ''}`} />
      </div>
      <div className="text-[9px] sm:text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-0.5 sm:mb-1 uppercase tracking-wide">{label}</div>
      <div className={`text-sm sm:text-base font-black ${config.text}`}>{value}</div>
    </div>
  );
};
